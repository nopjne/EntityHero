...
wxBEGIN_EVENT_TABLE (Edit, wxStyledTextCtrl)
    ...
    EVT_SET_FOCUS(Edit::MySetFocus)
    EVT_KILL_FOCUS(Edit::MyHandleKillFocus)
wxEND_EVENT_TABLE()
...
void Edit::MySetFocus(wxFocusEvent& Event)
{
    m_PreModification = GetText();
    Event.Skip();
}
...
void Edit::MyHandleKillFocus(wxFocusEvent& Event)
{
    if (Modified() != false) {
        ...
        wxString LineEndNormalization(GetText());
...
bool Edit::SetTextAndHighlight(wxString Text, EntityTreeModelNode* Node)
{
    SetText(Text);
    EmptyUndoBuffer();
