...
public:
    ...
    */
    ...
    bool StartObject(int Flags) {
        ...
        bool result;
        size_t size = Base::level_stack_.GetSize();
        if (size > (2 * sizeof(void*))) {
            result = Base::WriteStartObject();
        } else {
            result = true;
        }
    ...
    bool EndObject(int Flags, SizeType memberCount = 0) {
        (void)memberCount;
        ...
        bool empty = false;
        if ((Flags & 0x10000) == 0) {
            empty = Base::level_stack_.template Pop<typename Base::Level>(1)->valueCount == 0;
        }
...
protected:
    void PrettyPrefix(Type type, bool PreventSpace = false) {
        (void)type;
            ...
            if (level->inArray) {
                ...
                if (level->valueCount > 0) {
                    if (level->valueCount % 2 == 0) {
                        ...
                        bool NextBasicType = false;
                        size_t size = Base::level_stack_.GetSize();
                        if (PreventSpace == false) {
                            Base::os_->Put(' ');
                        }

                        if (Base::level_stack_.GetSize() > (2 * sizeof(void*)) && NextBasicType) {
                            Base::os_->Put('=');
                            Base::os_->Put(' ');
                        }
                    }
                    ...
                    if (Base::level_stack_.GetSize() > (2 * sizeof(void*))) {
                        Base::os_->Put('\n');
                    }
