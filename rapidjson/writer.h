...
public:
    ...
    static const int kDefaultMaxDecimalPlaces = 7;
...
protected:
    ...
    bool hideStatementEnd;

    void PostPretty() {
        if (hideStatementEnd == false) {
            PutReserve(*os_, 1);
            PutUnsafe(*os_, ';');
        }
    ...
    bool WriteNull()  {
        PutReserve(*os_, 5);
        PutUnsafe(*os_, 'N'); PutUnsafe(*os_, 'U'); PutUnsafe(*os_, 'L'); PutUnsafe(*os_, 'L'); PutUnsafe(*os_, ';'); return true;
        PostPretty();
    }
    ...
    bool WriteDouble(double d) {
        ...
        //int ptDec;
        //int ptSign;
        //_fcvt_s(buffer, d, maxDecimalPlaces_, &ptDec, &ptSign);
        //sprintf_s(fbuffer, "%.6f", d);
        sprintf_s(gbuffer, "%.11g", d);
        char *buffer = gbuffer;
        //if (strlen(fbuffer) > strlen(gbuffer)) {
        //    buffer = fbuffer;
        //}
...
#undef Z16
        ...
        bool specialKeyword = false;
        // TODO: The below can be done a lot cleaner if the additional information was stored along with the nodes in the object stack.
        //       Some have been integrated into flags, others have not.
        if ((memcmp(stringBuffer, "entity\0", sizeof("entity\0")) == 0) ||
            (memcmp(stringBuffer, "entityDef ", sizeof("entityDef")) == 0) ||
